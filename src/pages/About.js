      ...
      <section className="section" aria-labelledby="mission-heading">
        <div className="container about-mission">
          ...
          <div className="about-mission__visual">
            {[
              { card: '', icon: '🏢', title: 'Entreprises privées', desc: 'Solutions sur mesure pour vos enjeux environnementaux' },
              { card: '--orange', icon: '🏛️', title: 'Organisations publiques', desc: 'Accompagnement dans les démarches réglementaires' },
              { card: '--sky', icon: '👤', title: 'Porteurs de projets', desc: "Soutien individuel de l'idée à la réalisation" },
            ].map((item, i) => (
              <ScrollReveal key={i} direction="right" delay={i * 150}>
                <div className={`about-mission__card about-mission__card${item.card}`}>
      ...
      <section className="section section-alt" aria-labelledby="values-heading-about">
        <div className="container">
          ...
          <div className="about-values__grid">
            {[
              { bg: 'rgba(26,58,107,0.1)', color: 'var(--blue-dark)', icon: '🏆', title: 'Excellence', desc: "Nous visons l'excellence dans chaque mission en nous appuyant sur des expertises reconnues et des méthodes rigoureuses." },
              { bg: 'rgba(232,119,34,0.1)', color: 'var(--orange)', icon: '🌱', title: 'Durabilité', desc: "Chaque projet est conçu pour respecter l'environnement et contribuer au développement durable des générations futures." },
              { bg: 'rgba(46,127,193,0.1)', color: 'var(--blue-sky)', icon: '🔍', title: 'Rigueur', desc: "Nos études et conseils s'appuient sur des analyses rigoureuses et des méthodes éprouvées par l'expérience." },
              { bg: 'rgba(26,58,107,0.1)', color: 'var(--blue-dark)', icon: '🤝', title: 'Proximité', desc: "Nous construisons des relations de confiance durables avec nos clients, fondées sur l'écoute et la transparence." },
            ].map((v, i) => (
              <ScrollReveal key={i} direction="up" delay={i * 100}>
                <div className="about-value">
                  <div className="about-value__icon" style={{ background: v.bg, color: v.color }}>{v.icon}</div>
                  <h3>{v.title}</h3>
                  <p>{v.desc}</p>
                </div>
              </ScrollReveal>
          ...
          <div className="about-timeline">
            {milestones.map((m, i) => (
              <ScrollReveal key={i} direction={i % 2 === 0 ? 'left' : 'right'} delay={i * 100}>
                <div className={`about-timeline__item${i % 2 === 0 ? '' : ' about-timeline__item--right'}`}>
